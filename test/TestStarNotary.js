...
it('lets 2 users exchange stars', async () => {
    ...
    const tokenId1 = 6;
    const tokenId2 = 7;
    await instance.createStar("Awesome star no. 1", tokenId1, { from: user1 });;
    await instance.createStar("Awesome star no. 2", tokenId2, { from: user2 });
...
it('lookUptokenIdToStarInfo test', async () => {
    ...
    const savedName = await instance.lookUptokenIdToStarInfo(tokenId);

    // 3. Verify if you Star name is the same
    assert.equal(savedName, "Awesome star!");
});